...
// $order_month = $call->query("SELECT SUM(trx_socmed.price) AS tamount, count(trx_socmed.id) AS tcount, trx_socmed.user, users.name FROM trx_socmed JOIN users ON trx_socmed.user = users.username WHERE trx_socmed.status IN ('processing','partial','success') GROUP BY trx_socmed.user ORDER BY tamount DESC LIMIT 10");
$order_month = $call->query("SELECT SUM(trx_socmed.price) AS tamount, count(trx_socmed.id) AS tcount, trx_socmed.user, users.name FROM trx_socmed JOIN users ON trx_socmed.user = users.username WHERE MONTH(trx_socmed.date_cr) = '".date('m')."' AND YEAR(trx_socmed.date_cr) = '".date('Y')."' AND trx_socmed.status IN ('processing','partial','success') GROUP BY trx_socmed.user ORDER BY tamount DESC LIMIT 10");
...
$order_pro = $call->query("SELECT srv_socmed.id, SUM(trx_socmed.price) AS tamount, count(trx_socmed.id) AS tcount, trx_socmed.name FROM trx_socmed JOIN srv_socmed ON trx_socmed.name = srv_socmed.name WHERE MONTH(trx_socmed.date_cr) = '".date('m')."' AND YEAR(trx_socmed.date_cr) = '".date('Y')."' AND trx_socmed.status IN ('processing','partial','success') GROUP BY trx_socmed.name ORDER BY tamount DESC LIMIT 10");
// $order_pro = $call->query("SELECT srv_socmed.id, SUM(trx_socmed.price) AS tamount, count(trx_socmed.id) AS tcount, trx_socmed.name FROM trx_socmed JOIN srv_socmed ON trx_socmed.name = srv_socmed.name WHERE trx_socmed.status IN ('processing','partial','success') GROUP BY trx_socmed.name ORDER BY tamount DESC LIMIT 10");
?>
...
<div class="row">
    <div class="col-12 text-center mb-2">
        <h3><i class="fas fa-award fa-fw"></i>PENGGUNA TERATAS<i class="fas fa-award fa-fw"></i></h3>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="header-title">
                    <i class="feather icon-award mr-1"></i>TOP 10 Hari ini
                </h5>
    ...
    <div class="col-12 col-md-6">
        <div class="card">
            <div class="card-body">
                <h5 class="header-title">
                    <i class="feather icon-award mr-1"></i>TOP 10 dibulan ini
                </h5>
                <div class="table-responsive">
                    <table class="table table-striped table-bordered mb-0">
                        <thead>
    ...
    <div class="col-12 col-md-10 offset-md-1">
        <div class="card">
            <div class="card-body">
                <h5 class="header-title">
                    <i class="feather icon-award mr-1 info"></i>TOP 13 Layanan bulan ini
                </h5>
                <div class="table-responsive">
                    <table class="table table-striped table-bordered mb-0">
                        <thead>
                        ...
                        <tbody>
                            <? $rank = 1;
                            while ($row = $order_pro->fetch_assoc()) { ?>
                                <tr>
                                    <td class="text-center"><?= $rank ?></td>
