        ...
        <div class="table-responsive">
            <table class="table table-bordered" style="border:1px solid #dee2e6;">
                <tbody>
                    ...
                    <tr><th>Order ID</th><td><?= $row['wid'] ?></td></tr>
                    <tr><th>User</th><td><?= $row['user'] ?></td></tr>
                    <tr><th>Layanan</th><td><?= $row['name'] ?></td></tr>
                    <tr><th>Data</th><td><?= $row['data'] ?></td></tr>
                    <tr><th>Note</th><td><?= nl2br($row['note']) ?></td></tr>
                    <tr><th>Jumlah Order</th><td><?= currency($row['amount']) ?></td></tr>
                    <tr><th>Price</th><td><?= 'Rp '.currency($row['price']) ?></td></tr>
                    <tr><th>Profit</th><td><?= 'Rp '.currency($row['profit']) ?></td></tr>
                    <tr><th>Jumlah Awal</th><td><?= currency($row['count']) ?></td></tr>
                    <tr><th>Jumlah Kurang</th><td><?= currency($row['remain']) ?></td></tr>
                    ...
                    <tr><th>Order dari</th><td><?= $get_place ?></td></tr>
                    <tr><th>Tanggal Order</th><td><?= format_date('en',$row['date_cr']) ?></td></tr>
                    <tr><th>Terakhir Update</th><td><?= format_date('en',$row['date_up']) ?></td></tr>
                    ...
                    <? ################################################################### ?>
                    <tr><th>Order ID</th><td><?= $row['wid'] ?></td></tr>
                    <tr><th>ID Layanan</th><td><?= $call->query("SELECT id, name FROM srv_socmed WHERE name = '".$row['name']."'")->fetch_assoc()['id'] ?: "Layanan Kosong" ?></td></tr>
                    <?php  if($row['provider'] == 'SMMRAJA'){?>
                    <tr><th>Server</th><td>Server 2</td></tr>
                    <?php } else if ($row['provider'] == 'BIG'){?>
                    <tr><th>Server</th><td>Server 3</td></tr>
                    <?php } else { ?>
                    <tr><th>Server</th><td>Server 1</td></tr>
                    <?php } ?>
                    <tr><th>Layanan</th><td><?= $row['name'] ?></td></tr>
                    <tr><th>Data</th><td><?= $row['data'] ?></td></tr>
                    <tr><th>Catatan</th><td><?= nl2br($row['note']) ?></td></tr>
                    <tr><th>Jumlah Order</th><td><?= currency($row['amount']) ?></td></tr>
                    <tr><th>Harga</th><td><?= 'Rp '.currency($row['price']) ?></td></tr>
                    <tr><th>Jumlah Awal</th><td><?= currency($row['count']) ?></td></tr>
                    <tr><th>Jumlah Kurang</th><td><?= currency($row['remain']) ?></td></tr>
                    <tr><th>Status</th><td><?= $get_ic ?></tr>
                    <tr><th>Order Dari</th><td><?= $get_place ?></td></tr>
                    <tr><th>Tangal Order</th><td><?= format_date('en',$row['date_cr']) ?></td></tr>
                    <tr><th>Terakhir Update</th><td><?= format_date('en',$row['date_up']) ?></td></tr>
                    <tr><th>Waktu Proses</th><td><?= timeProcess($row['date_cr'],$row['date_up']) ?></td></tr>
                    <? } ?>
                </tbody>
