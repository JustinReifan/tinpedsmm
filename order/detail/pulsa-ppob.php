...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    $search = $call->query("SELECT * FROM trx_ppob WHERE wid = '$get_tid'");
        ...
        <div class="table-responsive">
            <table class="table table-bordered" style="border:1px solid #dee2e6;">
                <tbody>
                    ...
                    <tr><th>Status</th><td><?= $get_ic ?></tr>
                    <tr><th>Place From</th><td><?= $get_place ?></td></tr>
                    <tr><th>Tanggal pemesanan</th><td><?= format_date('en',$row['date_cr']) ?></td></tr>
                    <tr><th>Pembaharuan Terakhir</th><td><?= format_date('en',$row['date_up']) ?></td></tr>
                    <tr><th>Proses</th><td><?= timeProcess($row['date_cr'],$row['date_up']) ?></td></tr>
                    <? } ?>
                </tbody>
            </table>
        </div>
