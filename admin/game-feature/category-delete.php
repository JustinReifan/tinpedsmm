...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    $search = $call->query("SELECT * FROM category WHERE id = '$code' AND `order` = 'game'");
...
<form method="POST">
    ...
    <hr>
    <? if($call->query("SELECT id FROM srv_game WHERE game = '".$data['code']."'")->num_rows > 0) { ?>
    <h4>Detail</h4>
    <div style="font-size:0.8rem;display:inline;">
        Kategori ini mempunyai <?= currency($call->query("SELECT id FROM srv_game WHERE game = '".$data['code']."'")->num_rows) ?> layanan, apakah anda ingin menghapusnya juga?
