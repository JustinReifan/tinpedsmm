...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    $search = $call->query("SELECT * FROM srv_game WHERE id = '$get_id'");
...
<form method="POST">
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="category">
            <?php
            $s = $call->query("SELECT code, name FROM category WHERE `order` = 'game' ORDER BY name ASC");
            while($r = $s->fetch_assoc()) {
                print select_opt($row['game'], $r['code'], $r['name']);
            }
            ?>
        </select>
    </div>
    ...
    <div class="form-group">
        <label>Server</label>
        <input type="text" class="form-control" name="server" value="<?= $row['server'] ?>">
    </div>
    <div class="form-group">
        <label>Price</label>
    ...
    <div class="row">
        ...
        <div class="form-group col-6">
            <button type="submit" name="serviceg_edit" class="btn btn-primary btn-block"> EDIT </button>
        </div>
    </div>
</form>
<?php
