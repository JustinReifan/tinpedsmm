...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    $data_category = $call->query("SELECT * FROM category WHERE code = '".$row['game']."' AND `order` = 'game'");
    if($data_category->num_rows == 1) $data_category = $data_category->fetch_assoc();
    $nama_category = (is_array($data_category)) ? $data_category['name'] : $row['game'];
?>
<form method="POST">
    ...
    <div class="row">
        ...
        <div class="form-group col-6">
            <button type="submit" name="serviceg_delete" class="btn btn-primary btn-block"> DELETE </button>
        </div>
    </div>
</form>
<?php
