    ...
    <div class="row">
        ...
        <div class="col-md-4 col-12">
            <div class="card">
                <div class="card-header d-flex align-items-start pb-0">
                    <div>
                        <h2 class="text-bold-700 mb-0"><?= currency($call->query("SELECT id FROM trx_game WHERE status = 'error'")->num_rows) ?></h2>
                        <p>Error</p>
                    </div>
        ...
        <div class="col-md-4 col-12">
            <div class="card">
                <div class="card-header d-flex align-items-start pb-0">
                    <div>
                        <h2 class="text-bold-700 mb-0"><?= currency($call->query("SELECT id FROM trx_game WHERE status = 'waiting'")->num_rows) ?></h2>
                        <p>Waiting</p>
                    </div>
        ...
        <div class="col-md-4 col-12">
            <div class="card">
                <div class="card-header d-flex align-items-start pb-0">
                  <div>
                      <h2 class="text-bold-700 mb-0"><?= currency($call->query("SELECT id FROM trx_game WHERE status = 'success'")->num_rows) ?></h2>
                      <p>Success</p>
                  </div>
...
<?= datatable(['url' => ajaxlib('table/transaction-admin?__s=3'),'sort' => 0,'type' => 'desc']) ?>
<script type="text/javascript">
function ShennStOrder(wid,status) {
	$.ajax({
	    type: 'POST',
	    data: 'id=' + wid + '&data=' + status + '&csrf_token=' + csrf_key,
	    url: base_url + 'admin/game-feature/transaction-status',
		dataType: 'html',
