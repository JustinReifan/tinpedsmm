...
?>
<form method="POST">
    <input type="hidden" id="web_token" name="web_token" value="<?= base64_encode('game') ?>">
    ...
    <div class="form-group d-none" id="selcategory">
        ...
        <select class="form-control" name="clear_category">
            ...
            <?php
            $s = $call->query("SELECT * FROM category WHERE `order` = 'game'");
            while($r = $s->fetch_assoc()) {
                print '<option value="'.$r['code'].'">['.currency(squery("SELECT id FROM srv_game WHERE game = '".$r['code']."'")->num_rows).'] '.$r['name'].'</option>';
            }
            ?>
        </select>
    </div>
    <div class="form-group d-none" id="selprovider">
        ...
        <select class="form-control" name="clear_provider">
            ...
            while($r = $s->fetch_assoc()) {
                print '<option value="'.$r['code'].'">['.currency(squery("SELECT id FROM srv_game WHERE provider = '".$r['code']."'")->num_rows).'] '.$r['name'].'</option>';
            }
            ?>
        </select>
    </div>
