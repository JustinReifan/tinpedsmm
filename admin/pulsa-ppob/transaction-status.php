...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if(!in_array($dst,['error','waiting','success'])) exit('No direct script access allowed!');
    $search = $call->query("SELECT * FROM trx_ppob WHERE wid = '".filter($_POST['id'])."'");
    if($search->num_rows == 0) exit(sessResult(false,'Transaction not found.'));
    $data = $search->fetch_assoc();
    
    if(in_array($data['status'],['error'])) {
        print sessResult(false,ucfirst($data['status']).' status cannot be changed.');
    } else {
        if($call->query("UPDATE trx_ppob SET status = '$dst', date_up = '$date $time' WHERE wid = '$did'") == true) {
