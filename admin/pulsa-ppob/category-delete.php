...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    $search = $call->query("SELECT * FROM category WHERE id = '$code'");
...
<form method="POST">
    ...
    <div style="font-size:0.8rem;display:inline;">Name<span class="justify-content-end" style="float:right;font-size:0.8rem;"><?= $data['name'] ?></span></div><br>
    <div style="font-size:0.8rem;display:inline;">Type<span class="justify-content-end" style="float:right;font-size:0.8rem;"><?= $new ?></span></div>
    <hr>
    <? if($call->query("SELECT id FROM srv_ppob WHERE type = '".$data['type']."' AND brand = '".$data['code']."'")->num_rows > 0) { ?>
    <h4>Detail</h4>
    <div style="font-size:0.8rem;display:inline;">
        Kategori ini mempunyai <?= currency($call->query("SELECT id FROM srv_ppob WHERE type = '".$data['type']."' AND brand = '".$data['code']."'")->num_rows) ?> layanan, apakah anda ingin menghapusnya juga?
