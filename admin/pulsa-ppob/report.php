        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-end">
                    ...
                    <a href="javascript:;" <?= onclick_modal('Detail Report', base_url('admin/pulsa-ppob/report-detail'), 'lg') ?>>
                ...
                <div class="card-body card-dashboard">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered zero-configuration" width="100%">
                            ...
                            <tbody>
                                <tr>
                                    <td><?= currency(squery("SELECT id FROM trx_ppob WHERE status = 'success' AND DATE(date_cr) BETWEEN '$SDate' AND '$EDate'")->num_rows) ?></td>
                                    <td><?= 'Rp '.currency(squery("SELECT SUM(price) AS x FROM trx_ppob WHERE status = 'success' AND DATE(date_cr) BETWEEN '$SDate' AND '$EDate'")->fetch_assoc()['x']).',-' ?></td>
                                    <td><?= 'Rp '.currency(squery("SELECT SUM(profit) AS x FROM trx_ppob WHERE status = 'success' AND DATE(date_cr) BETWEEN '$SDate' AND '$EDate'")->fetch_assoc()['x']).',-' ?></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
?>
                                <tr>
                                    <td><?= $row['name'] ?></td>
                                    <td><?= currency(squery("SELECT id FROM trx_ppob WHERE provider = '".$row['code']."' AND status = 'success' AND DATE(date_cr) BETWEEN '$SDate' AND '$EDate'")->num_rows) ?></td>
                                    <td><?= 'Rp '.currency(squery("SELECT SUM(price) AS x FROM trx_ppob WHERE provider = '".$row['code']."' AND status = 'success' AND DATE(date_cr) BETWEEN '$SDate' AND '$EDate'")->fetch_assoc()['x']).',-' ?></td>
                                    <td><?= 'Rp '.currency(squery("SELECT SUM(profit) AS x FROM trx_ppob WHERE provider = '".$row['code']."' AND status = 'success' AND DATE(date_cr) BETWEEN '$SDate' AND '$EDate'")->fetch_assoc()['x']).',-' ?></td>
                                </tr>
<? } ?>
                            </tbody>
                        </table>
        ...
        series: [
            { name: "Total Profit", data: [<?php
                for($i = 0; $i < count($dateList); $i++) {
                    $sum = squery("SELECT SUM(profit) AS x FROM trx_ppob WHERE status = 'success' AND date_cr LIKE '".$dateList[$i]."%'")->fetch_assoc()['x'];
                    print ($sum > 0) ? "$sum," : '0,';
                }
            ?>] },
            <? while($row = $search->fetch_assoc()) { ?>
            { name: "<?= $row['name'] ?>", data: [<?php
                for($i = 0; $i < count($dateList); $i++) {
                    $sum = squery("SELECT SUM(profit) AS x FROM trx_ppob WHERE provider = '".$row['code']."' AND status = 'success' AND date_cr LIKE '".$dateList[$i]."%'")->fetch_assoc()['x'];
                    print ($sum > 0) ? "$sum," : '0,';
                }
            ?>] },
            <? } ?>
