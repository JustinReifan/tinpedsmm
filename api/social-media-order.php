...
if($check_service->num_rows > 0) {
    ...
    // $profit_db = (in_array($data_user['level'],['Admin','Premium','H2H'])) ? $data_srv['premium'] : $data_srv['basic'];
        if($data_user['level'] == 'Admin' || $data_user['level'] == 'Premium'){
            $profit_db = $data_srv['premium'];
        } else if($data_user['level'] == 'H2H'){
            $profit_db = $data_srv['h2h'];
        } else{
            $profit_db = $data_srv['basic'];
        }
...
$inp = $call->query("INSERT INTO trx_socmed VALUES (null,'$ShennTRX','','$username','$service','$post_tgt','','','','$post_qty','$price','$point','$profit','0','0','waiting','0','0','API','$dtme','$dtme','$provider')");
