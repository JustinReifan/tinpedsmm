...
if(!isset($_SESSION['user'])) {
    if(isset($_COOKIE['token']) && isset($_COOKIE['ssid'])) {
        ...
        ShennXit(['type' => false,'message' => 'Diperlukan otentikasi, harap masuk terlebih dahulu.'],base_url('auth/login'));
    }
}
...
if(isset($_SESSION['user'])) {
    ...
    if($data_user['status'] <> 'active') {
        ...
        ShennXit(['type' => false,'message' => 'Harap Vrifikasi dulu alamat email Anda.'],base_url('account/profile'));
    } else if($equalUserPhone == false && $json_user['old_phone'] == '' && !isset($STOP_REDIRECT)) {
        ShennXit(['type' => false,'message' => 'Harap validasi nomor telepon Anda.'],base_url('account/profile'));
