...
if(isset($_SESSION['user'])) {
    ...
    if($call->query("SELECT * FROM users WHERE username = '$sess_username'")->num_rows == 1 && $call->query("SELECT * FROM users_cookie WHERE cookie = '$sess_cookie' AND username = '$sess_username'")->num_rows == 1) {
        ...
        $data_usercookie = $call->query("SELECT * FROM users_cookie WHERE cookie = '$sess_cookie' AND username = '$sess_username'")->fetch_assoc();
        ...
        if($_SESSION['user']['password'] <> $data_user['password']) {
            ShennXit(['type' => false,'message' => 'Kata sandi telah diubah, silakan masuk kembali.'],base_url('auth/logout'));
        }
    }
} else {
