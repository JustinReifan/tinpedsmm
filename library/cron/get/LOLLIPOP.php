...
$prov = $call->query("SELECT * FROM provider WHERE code = 'LOLLIPOP'");
        ...
        if($try['status'] == true) {
            if(count($try['data']) > 0) {
                for($i = 0; $i <= count($try['data'])-1; $i++) {
                    ...
                    $check_service = $call->query("SELECT * FROM srv_socmed WHERE pid = '$pid' AND provider = 'LOLLIPOP'");
                    ...
                    if($check_service->num_rows == 0) {
                        $call->query("INSERT INTO srv_socmed VALUES ('','$pid','$category','$name','$note','$price','$priceb','$pricep','$min','$max','available','','LOLLIPOP','$date $time')");
                        ...
                        if($data_service['price'] <> $price || $reset == true || $data_service['min'] <> $min || $data_service['max'] <> $max) {
                            ...
                            $call->query("UPDATE srv_socmed SET cid = '$category',name = '$name', price = '$price', basic = '$priceb', premium = '$pricep',h2h = '$prich', min = '$min', max = '$max',status = 'available', date_up = '$date $time', WHERE pid = '$pid' AND provider = 'LOLLIPOP'");
                            if($shows == true) print '<font color="green"><pre>';
