...
class helper
{
    ...
    public function statusM($x) {
        $y = strtolower($x);
        if(in_array($y,['failed','gagal','error','refund','refunded','Canceled'])) $str = 'Canceled';
        if(in_array($y,['partial','Partial'])) $str = 'partial';
        if(in_array($y,['pending','waiting','Pending'])) $str = 'pending';
        if(in_array($y,['processing','proses','Processing'])) $str = 'processing';
        if(in_array($y,['success','sukses','Success','berhasil','Completed'])) $str = "success";
        return (!isset($str)) ? "Waiting" : $str;
    }
