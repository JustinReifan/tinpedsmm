...
function filter_comments($input){
    $clean=htmlentities(strip_tags(addslashes(trim($input))));
    ...
    $clean=str_replace('>','\>',$clean);
    $clean=str_replace('<','\<',$clean);
    $clean = str_replace('alert(','',$clean);
    $clean = str_replace('javascript::','',$clean);
    $clean = str_replace('document.cookie','',$clean);
    return $clean;
}
