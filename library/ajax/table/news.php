...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    if(!in_array($ctype, ['1','2','3','4','5'])) exit("No direct script access allowed!");
    ...
    } else if ($ctype == '5' && $data_user['level'] == 'Admin'){
        $table = 'rekomendasi';
        $primaryKey = 'no_id';
        $columns = [
            ['db' => 'nomor', 'dt' => 0],
            ['db' => 'name', 'dt' => 1, 'formatter' => function($i) {
                return $i;
            }],
            ['db' => 'no_id', 'dt' => 2, 'formatter' => function($i) {
                $shenn_edit1 = base_url('admin/input-rekomendasi/edit?s='.$i);
                $shenn_delete1 = base_url('admin/input-rekomendasi/delete?s='.$i);
                return '<center>
    <a href="javascript:;" class="font-medium-5 mr-1" style="text-decoration:none;" '.onclick_href($shenn_edit1).'>
        <i class="feather icon-edit"></i>
    </a>
    <a href="javascript:;" class="font-medium-5 ml-1" style="text-decoration:none;" '.onclick_modal('Delete News / Info',$shenn_delete1,'lg').'>
        <i class="feather icon-trash"></i>
    </a>
</center>';
            }],
