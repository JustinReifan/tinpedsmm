...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
        ...
        $columns = [
            ...
            ['db' => 'data', 'dt' => 3, 'formatter' => function($i) {
                return '<div style="vertical-align:middle;"><div class="input-group"><input type="text" value="'.$i.'" class="form-control form-control-sm" readonly>
                        <div class="input-group-append"><button class="btn btn-sm text-white bg-gradient-warning fw-bold copy" data-clipboard-text="'.$i.'"><i class="far fa-copy">
                        </i></button></div></div></div>';
            }],
            ['db' => 'tid','dt' => 4, 'formatter' => function($i,$a) {
                if($a['status'] == 'error') $ic = 'bg-gradient-danger';
                if($a['status'] == 'waiting') $ic = 'bg-gradient-warning';
                if($a['status'] == 'success') $ic = 'bg-gradient-success';
    ...
    <a href="javascript:;" class="font-medium-5 ml-1" style="text-decoration:none;" '.onclick_modal('Edit Order',base_url('admin/pulsa-ppob/transaction-edit?s='.$i),'lg').'>
        ...
        $columns = [
            ...
            ['db' => 'tid','dt' => 4, 'formatter' => function($i,$a) {
                if($a['status'] == 'error') $ic = 'bg-gradient-danger';
                if($a['status'] == 'partial') $ic = 'bg-gradient-danger';
                if($a['status'] == 'waiting') $ic = 'bg-gradient-warning';
                if($a['status'] == 'processing') $ic = 'bg-gradient-info';
                if($a['status'] == 'success') $ic = 'bg-gradient-success';
        ...
        $columns = [
            ...
            ['db' => 'data', 'dt' => 3, 'formatter' => function($i,$a) {
                return '<div style="vertical-align:middle;"><div class="input-group"><input type="text" value="'.$i.' ('.$a['zone'].')" class="form-control form-control-sm" readonly>
                        <div class="input-group-append"><button class="btn btn-sm text-white bg-gradient-warning fw-bold copy" data-clipboard-text="'.$i.' ('.$a['zone'].')"><i class="far fa-copy">
                        </i></button></div></div></div>';
            }],
            ['db' => 'tid','dt' => 4, 'formatter' => function($i,$a) {
                if($a['status'] == 'error') $ic = 'bg-gradient-danger';
