...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    if($ctype == '1') { // Pulsa PPOB
        ...
        ['db' => 'status','dt' => 'status'],
           ...
           ['db' => 'date_up' , 'dt' => 3, 'formatter' => function($i,$a) {
                $format = $i;
                $dateUp = $a;
                return "<center>".timeProcess($format,$dateUp['date_cr'])."</center>";
            }],
        ];
        ...
        $columns = [
            ...
            ['db' => 'data','dt' => 2, 'formatter' => function($i) {
                return ''.substr($i,-10,2).'xxxx';
            }],
                
            ['db' => 'amount','dt' => 3, 'formatter' => function($i) {
                return '<center>'.currency($i).'</center>';
            }],
            
            ['db' => 'date_up' , 'dt' => 4, 'formatter' => function($i,$a) {
                    $format = $i;
                    $dateUp = $a;
                    return "<center>".timeProcess($format,$dateUp['date_cr'])."</center>";
            }],
