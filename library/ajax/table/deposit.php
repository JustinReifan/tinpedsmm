...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    if($ctype == '1') {
        ...
        $columns = [
            ...
            ['db' => 'amount', 'dt' => 1, 'formatter' => function($i,$a) {
                if($a['status'] == 'cancelled') $ic = 'far fa-times-circle text-danger';
                if($a['status'] == 'unpaid') $ic = 'fas fa-circle-notch fa-spin text-warning';
                if($a['status'] == 'paid') $ic = 'far fa-check-circle text-success';
            ...
            ['db' => 'note', 'dt' => 2, 'formatter' => function($i,$a) {
                if(isset(explode(' A/n ',$i)[1])) {
                    if(in_array($a['payment'],['telkomsel','axiata'])) {
                        return explode(' A/n ',$i)[0];
                    } else {
                        return explode(' A/n ',$i)[0].'<br>'.explode(' A/n ',$i)[1];
                    }
                } else {
                    return $i;
                }
    ...
    <div class="dropdown">
        <button class="btn bg-gradient-warning dropdown-toggle" type="button" id="drop1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Unpaid</button>
        <div class="dropdown-menu" aria-labelledby="drop1">
            <a class="dropdown-item" href="javascript:;" onclick="ShennStDeposit(\''.$t_paid.'\')">Dibayar</a>
            <a class="dropdown-item" href="javascript:;" onclick="ShennStDeposit(\''.$t_cancel.'\')">Batal</a>
        </div>
    </div>
</center>';
    ...
    <div class="dropdown">
        <button class="btn bg-gradient-success dropdown-toggle" type="button" disabled>Dibayar</button>
    </div>
</center>';
                                        
                if($a['status'] == 'cancelled') return '<center>
    <div class="dropdown">
        <button class="btn bg-gradient-danger dropdown-toggle" type="button" disabled>Batal</button>
    </div>
</center>';
            }],
        ];
        ...
        $columns = [
            ...
            ['db' => 'id',  'dt' => 5, 'formatter' => function($i) {
                $shenn_edit = base_url('admin/deposit/method-edit?s='.base64_encode($i));
                $shenn_delete = base_url('admin/deposit/method-delete?s='.base64_encode($i));
                return "<center>
    <a href=\"javascript:;\" class=\"font-medium-5 mr-1\" style=\"text-decoration:none;\" ".onclick_modal('Edit Method',$shenn_edit,'lg').">
        <i class=\"feather icon-edit\"></i>
    </a>
    <a href=\"javascript:;\" class=\"font-medium-5 ml-1\" style=\"text-decoration:none;\" ".onclick_modal('Delete Method',$shenn_delete,'lg').">
