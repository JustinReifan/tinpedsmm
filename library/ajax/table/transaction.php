    ...
    if($ctype == '1') { // Pulsa PPOB
        ...
        $columns = [
            ...
            ['db' => 'name', 'dt' => 1, 'formatter' => function($i,$a) {
                ...
                return $service.'<br><medium class="text-primary"><i class="'.$ic.'"></i> <shenn class="copy" style="cursor:pointer;" title="Click to copy!" data-clipboard-text="'.$a['wid'].'">Trx: '.$a['wid'].'</shenn></medium>';
            }],
            ['db' => 'data', 'dt' => 2, 'formatter' => function($i) {
                return '<div style="vertical-align:middle;"><div class="input-group"><input type="text" value="'.$i.'" class="form-control form-control-sm" readonly>
                        <div class="input-group-append"><button class="btn btn-sm text-white bg-gradient-success fw-bold copy" data-clipboard-text="'.$i.'">
            ...
            ['db' => 'wid','dt' => 4, 'formatter' => function($i,$a) {
                return '<center><a href="javascript:;" class="btn btn-info text-white" onclick="modal(\'Order Details\',\''.base_url('order/detail/pulsa-ppob?id='.$i).'\',\'lg\')"><i class="fa fa-info-circle"></i></a></center>';
            }],
        ];
        ...
        $columns = [
            ...
            ['db' => 'name', 'dt' => 1, 'formatter' => function($i,$a) {
                ...
                if($a['status'] == 'success') $ic = 'far fa-check-circle text-success';
                $service = (strlen($i) > 26) ? substr($i,0,23).'...' : $i;
                return $service.'<br><medium class="text-primary"><i class="'.$ic.'"></i> <shenn class="font-bold" >'.$a['status'].'</shenn></medium>';
            }],
            ...
            ['db' => 'wid','dt' => 5, 'formatter' => function($i,$a) {
                return '<center><a href="javascript:;" class="btn btn-info text-white" onclick="modal(\'Order Details\',\''.base_url('order/detail/social-media?id='.$i).'\',\'lg\')"><i class="fas fa-info-circle"></i></a></center>';
            }],
        ];
        $sql_details = ['user' => $aiy['user'],'pass' => $aiy['pass'],'db' => $aiy['name'],'host' => $aiy['host']];
        $joinQuery = null;
        ...
        $columns = [
            ...
            ['db' => 'wid','dt' => 4, 'formatter' => function($i,$a) {
                return '<center><a href="javascript:;" class="btn btn-primary text-white" onclick="modal(\'Order Details\',\''.base_url('order/detail/game-feature?id='.$i).'\',\'lg\')"><i class="fas fa-info-circle"></i></a></center>';
            }],
        ];
        $sql_details = ['user' => $aiy['user'],'pass' => $aiy['pass'],'db' => $aiy['name'],'host' => $aiy['host']];
        $joinQuery = null;
