...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    if($ctype == '1') {
        ...
        $columns = [
            ...
            // ['db' => 'user',  'dt' => 3, 'formatter' => function($i,$a) {
            //      $data = squery("SELECT balance,username FROM users WHERE username = '$i'");
            //     if($data->num_rows == 1) $data = $data->fetch_assoc();
            //     return '<center><b class="badge badge-success badge-md">Rp '.currency($data['balance']).'</b></center>';
            // }],
