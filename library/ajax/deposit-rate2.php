...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    if(!isset($_POST['quantity']) || !isset($_POST['method'])) exit('Tidak ada akses skrip langsung yang diizinkan!');
    if(!is_numeric($_POST['quantity']) || !is_numeric($_POST['method'])) exit('Tidak ada akses skrip langsung yang diizinkan!');
    if($call->query("SELECT * FROM deposit_method WHERE id = '".filter($_POST['method'])."'")->num_rows == 0) exit('Tidak ada akses skrip langsung yang diizinkan!');
	...
	exit('NTidak ada akses skrip langsung yang diizinkan!');
}