...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($data_user)) {
    ...
    if (!isset($_POST['srvid'])) exit('No direct script access allowed!');
    if (empty($_POST['srvid'])) exit(json_encode([
        'msg' => [
            'category' => '',
            'service_id' => '',
            'service_name' => '',
            ...
            'min' => '0',
            'max' => '0',
            'note' => 'Harap Masukkan ID Layanan',
            
            'refill' => '',
        ]
    ]));

 $qlayanan = $call->query("SELECT * FROM srv_socmed WHERE id = '".filter($_POST['srvid'])."'");
    ...
    $search = $call->query("SELECT * FROM srv_socmed WHERE id = '" . filter($_POST['srvid']) . "' AND status = 'available'");
    if ($search->num_rows == 1) :
        $row = $search->fetch_assoc();
        $pricing = (in_array($data_user['level'], ['Admin', 'Premium', 'Special'])) ? ($data_user['level'] == 'Special' ? $row['special'] : $row['premium']) : $row['basic'];
        print json_encode([
            'msg' => [
                'category' => $row['cid'],
                'service_id' => $row['id'],
        ...
        print json_encode([
            'msg' => [
                'category' => 'Layanan Tidak Tersedia!',
                'service_id' => '',
                'service_name' => '',
                'price' => '0',
                'rate' => '0',
