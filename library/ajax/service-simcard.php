        ...
        if($search->num_rows == 0) {
            ...
            while($row = $search->fetch_assoc()) {
                ...
                $out_srv .= '<div class="col-md-4" onclick="'.$onclick.'"><label class="custom control-label"><input type="radio" name="shennboku" class="card-input-element d-no'.
                            'ne" value="shennboku"><div class="card card-body"><div style="font-size:1rem;display:inline;height:auto !important;">'.$row['name'].''.
                            '</div><span style="font-size:0.9rem;">Rp '.currency($priceny).',-</span></div></label></div>';
            }
        }
        print json_encode(['service' => $out_srv,'class' => $class]);
    } else {
        exit(json_encode(['service' => err('Kartu SIM tidak terdeteksi.'),'class' => '']));
    }
} else {
