    ...
    if($search->num_rows == 0) {
        ...
        while($row = $search->fetch_assoc()) {
            ...
            $out_srv .= '<div class="col-md-6" onclick="'.$onclick.'"><label class="custom control-label"><input type="radio" name="shennboku" class="card-input-element d-no'.
                        'ne" value="shennboku"><div class="card card-body"><div style="font-size:1.1rem;display:inline;">'.$row['name'].'<span class="justify-content-end" st'.
                        'yle="float:right;font-size:0.9rem;">Rp '.currency($priceny).',-</span></div><span style="font-size:0.8rem;">'.$row['note'].'</span></div></label></div>';
        }
        exit($out_srv);
    }
} else {
