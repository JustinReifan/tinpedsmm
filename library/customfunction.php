...
function timeProcess($start, $end)
{
    if ($start === $end) {
        return '0 detik';
    } else {
        ...
        $years = ($years >= 1) ? "$years tahun, " : '';
        $months = ($months >= 1) ? "$months bulan, " : '';
        $days = ($days >= 1) ? "$days hari, " : '';
        $hours = ($hours >= 1) ? "$hours jam, " : '';
        $minutes = ($minutes >= 1) ? "$minutes menit, " : '';
        $seconds = ($seconds >= 1) ? "$seconds detik" : '';
        $shennboku = "$years$months$days$hours$minutes$seconds";

        $out = '';
        $array = explode(', ', $shennboku);
        for ($i = 0; $i <= count($array) - 1; $i++) {
            if ($i !== (count($array) - 1)) {
                $out .= ($i == (count($array) - 2)) ? $array[$i] : $array[$i] . ' ';
            } else if (stristr($array[$i], 'detik')) {
                $out .= ' ' . $array[$i];
            }
        }
        return $out;
...
function ul($x) {
    ...
    for($i = 0; $i <= count($x['content'])-1; $i++) {
        if(isset($x['content'][$i]['name'])) {
            ...
            $li .= '"><a href="'.$url.$x['content'][$i]['page'].'"><i class="feather icon-circle"></i><span class="menu-title" data-i18n="'.$x['content'][$i]['name'].'">'.$x['content'][$i]['name'].'</span></a>';
        }
    }
    $act = (stristr($li,'nav-item active')) ? ' active' : '';
    print '<li class="nav-item'.$act.'"><a href="#"><i class="'.$x['icon'].'"></i><span class="menu-title" data-i18n="'.$x['name'].'">'.$x['name'].'</span></a><ul class="menu-content">'.$li.'</ul></li>';
}
...
function li($name,$icon,$page = '') {
    ...
    $li .= '"><a href="'.$url.$page.'"><i class="'.$icon.'"></i><span class="menu-title" data-i18n="'.$name.'">'.$name.'</span></a>';
    print $li;
}
...
function ShennXit($x = false,$y = 'RotasiPedia') {
    $z = ($y == 'RotasiPedia') ? visited() : $y;
...
function LannStatusSocmed($__stasoc) {
  if ($__stasoc == "waiting") {
    return '<small><i class="fas fa-circle-notch fa-spin text-warning"></i> Pending</small>'; 
