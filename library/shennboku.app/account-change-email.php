...
if(isset($_GET['send']) && $_POST) {
    ...
    $data_users = $call->query("SELECT * FROM users WHERE username = '$sess_username'")->fetch_assoc();
    $apikey =  $p['apikey'];
    $sender = $p['userid'];
...
$WATL->sendMessage($apikey, $sender,  $data_users['phone'],
"
Hallo ".$data_users['name']."
Kode OTP Verifikasi Anda  : *".$New_OTP."*
Jika Anda tidak mengenali aktivitas ini, segera hubungi CS admin rotasipedia.
...
if(isset($_GET['resend']) && $_POST) {
    if($result_csrf == false) {
        ...
        if($send == true) {
            ...
            print sessResult(true, 'Kode konfirmasi telah berhasil dikirim ke Email Anda.');
        } else {
            print sessResult(false, 'Gagal Kirim OTP, silakan coba lagi nanti.');
        }
    }
    die;
}
