...
if(isset($_POST['cancel'])) {
    ...
    if($result_csrf == false) {
        ShennXit(['type' => false,'message' => 'Sistem eror, silahkan coba lagi nanti.']);
    } else if($_CONFIG['lock']['status'] == true) {
        ShennXit(['type' => false,'message' => $_CONFIG['lock']['reason']]);
    } else if(!$web_token || !$post_password) {
        ShennXit(['type' => false,'message' => 'Harap di isi.']);
    } else if($call->query("SELECT wid FROM deposit WHERE wid = '$web_token' AND status = 'unpaid'")->num_rows == false) {
        ShennXit(['type' => false,'message' => 'Invoice tidak ditemukan.']);
    } else if($check_password == false) {
        ShennXit(['type' => false,'message' => 'PIN atau Password salah.']);
    } else {
        if($call->query("UPDATE deposit SET status = 'cancelled' WHERE wid = '$web_token' AND status = 'unpaid'") == true) {
            ShennXit(['type' => true,'message' => 'Permintaan deposit berhasil dibatalkan.']);
        } else {
            ShennXit(['type' => false,'message' => 'Server kami sedang mengalami masalah, silahkan coba kembali nanti.']);
        }
    }
}
